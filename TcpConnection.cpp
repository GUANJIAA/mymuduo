...
{
    // 下面给channel设置相应的回调函数，poller给channel通知感兴趣的事件发送了，channel会回调相应的操作函数
...
void TcpConnection::sendInLoop(const void *data, size_t len)
{
    ...
    // 说明当前这一次write，并没有把数据全部发送出去，剩余数据需要保存到缓冲区当中，然后给channel
    // 注册epollout事件，poller发现tcp的发送缓冲区有空间，会通知相应的sock-channel，调用handleWrite回调方法
    // 也就是调用TCP Connection::handleWrite方法，把发送缓冲区中的数据全部发送完成
...
void TcpConnection::connectDestroyed()
{
    if (state_ == kConnected)
    {
        ...
        channel_->disableAll(); // 把channel的所有感兴趣的事情，从poller中del掉
        connectionCallback_(shared_from_this());
    }
...
void TcpConnection::handleRead(Timestamp receviceTime)
{
    ...
    if (n > 0)
    {
        // 已建立连接的用户，有可读事件发送了，调用用户传入的回调操作onMessage
